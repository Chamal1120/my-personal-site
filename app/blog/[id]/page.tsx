...
const speechBlockSelector = "h1, h2, h3, h4, h5, h6, p, li, blockquote"

// Splits the post into readable blocks for text-to-speech, skipping code blocks.
// Only the outermost matching block is kept so nested text isn't read twice.
const extractSpeechChunks = (html: string) => {
    const doc = new DOMParser().parseFromString(html, "text/html")
    doc.querySelectorAll("pre").forEach((pre) => pre.remove())

    return Array.from(doc.body.querySelectorAll(speechBlockSelector))
        .filter((el) => !el.parentElement?.closest(speechBlockSelector))
        ...
        const fetchBlogPost = async () => {
            try {
                ...
                if (blogPost.body_markdown) {
                    ...
                    setContent(parseHtmlToReact(html))
                    setSpeechChunks([
                        blogPost.title,
                        ...extractSpeechChunks(html),
                    ])
                }
            } catch (err) {
        ...
        return (
            <section className="w-full">
                ...
                >
                    <svg
                        aria-hidden="true"
                        className="h-8 w-8 animate-spin text-fg"
                        viewBox="0 0 100 101"
                        fill="none"
    ...
    return (
        <section className="w-full">
            <div className="w-full">
                ...
                <div className="mb-6 font-mono text-sm text-fg/60">
