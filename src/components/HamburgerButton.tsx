...
export default function HamburgerButton({ showNav, setShowNav }: NavProps) {
  return (
    ...
    >
      {/* Line 1 */}
      <span
        className={`block h-0.5 w-6 transform bg-ctp-yellow-dark transition-all duration-300 ${
      ...
      {/* Line 2 */}
      <span
        className={`block h-0.5 w-6 bg-ctp-yellow-dark transition-all duration-300 ${
          showNav ? "opacity-0" : ""
        }`}
      ></span>
